        ...
        match embedding_provider:
            ...
            case "openai":
                ...
                if "openai_api_base" not in embedding_kwargs and os.environ.get("OPENAI_BASE_URL"):
                    ...
                    # Third-party compatible APIs (e.g. DashScope/Qwen) only accept plain
                    # strings, not the token-array format that OpenAI's official API supports.
                    # Setting this to False makes langchain send raw text instead of token arrays.
                    embedding_kwargs.setdefault("check_embedding_ctx_length", False)
                    # DashScope limits each embedding request to at most 10 texts per batch,
                    # while the OpenAI default chunk_size is 1000. Cap it to stay within limits.
